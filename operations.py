...
def store_ordenes_salida_cria_produccion(conn, cursor, df, batch_size=500, max_workers=32):
    # Convert the DataFrame rows into a list of tuples for easy insertion
    data = [(  record[0]
             , record[1]
             , float(record[2] or 0)
    ...
    try:
        ...
        # Fetch existing id_sap from the database to determine which records to update or insert
        cursor.execute("SELECT trim(id_sap) FROM crias_ordenes_salida WHERE id_sap IN %s", (tuple(id_sap_list),))
        ...
        def batch_insert(batch):
            sql_insert = """
                INSERT INTO crias_ordenes_salida (id_sap, orden, cant_aves, status, creation_date, "granjaOrigenIdId", "granjaDestinoIdId", "transporteIdId", "galponIdId")
